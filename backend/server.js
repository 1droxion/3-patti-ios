...
function settle(room) {
  ...
  room.revealed = true;
  room.message = `${winner.name} wins ${room.pot} chips.`;
}
...
function maybeSettleLastStanding(room) {
  ...
  if (active.length === 1 && room.status === 'playing') {
    ...
    room.status = 'showdown';
    room.revealed = true;
    room.message = `${active[0].name} wins ${room.pot} chips. Everyone else packed.`;
  }
}
...
function joinRoom({ playerId, name, playerCount }) {
  ...
  let room = q.map(id => rooms.get(id)).find(r => r && r.status === 'waiting' && r.players.length < r.playerCount);
  if (!room) {
    room = {
      id: randomUUID(),
      playerCount,
  ...
  if (!room.players.some(p => p.id === playerId)) {
    room.players.push({ id: playerId, name: String(name || 'Guest').slice(0, 24), chips: 10000, folded: false, seen: false, cards: [] });
  }
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`3 Patti social server listening on http://0.0.0.0:${PORT}`);
});
